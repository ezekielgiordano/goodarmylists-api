...
class LizardmenContainer extends Component {
	constructor(props) {
		super(props)
		this.state = {
			listedUnits: [
	...
	determineIfGreyedOut(unitArray) {
		...
		for (i2 = 0; i2 < unitsInArmy.length; i2++) {
			...
			if (
				skinkCount < wouldBeMaximumCount * 2 ||
				saurusCount < wouldBeMaximumCount * 2
			) {
				locked = true
			}
			if (unitsInArmy[i2].name === 'Skinks (Lizardmen)') {
				if (saurusCount < wouldBeMaximumCount * 2) {
					locked = true
				} else {
					locked = false
				}
			}
			if (unitsInArmy[i2].name === 'Saurus (Lizardmen)') {
				if (skinkCount < wouldBeMaximumCount * 2) {
					locked = true
				} else {
					locked = false
				}
	...
	determineIfValidAfterPointIncrease(pointIncrease) {
		...
		if (maximumCountBefore === maximumCountAfter) {
			...
			for (i2 = 0; i2 < listedUnits.length; i2++) {
				if (listedUnits[i2].unit.name === 'Skinks (Lizardmen)') {
					skinkCount += listedUnits[i2].count
				}	
				if (listedUnits[i2].unit.name === 'Saurus (Lizardmen)') {
					saurusCount += listedUnits[i2].count
				}			
			}
			if (
				skinkCount > maximumCountAfter * 2 ||
				saurusCount > maximumCountAfter * 2
			) {
				valid = false
			} else {
				valid = true
	...
	removeUnit(unitToRemove) {
		...
		for (i2 = 0; i2 < listedUnits.length; i2++) {
			if (listedUnits[i2].unit.name === unitToRemove.unit.name) {
				if (
					unitToRemove.unit.name === 'Skinks (Lizardmen)' ||
					unitToRemove.unit.name === 'Saurus (Lizardmen)'
				) {
...
export default LizardmenContainer