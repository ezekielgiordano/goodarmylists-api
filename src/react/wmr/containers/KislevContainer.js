...
class KislevContainer extends Component {
	constructor(props) {
		super(props)
		this.state = {
			listedUnits: [
	...
	determineIfGreyedOut(unitArray) {
		...
		for (i2 = 0; i2 < unitArray.length; i2++) {
			if (unitArray[i2].unit.name === 'Horse Archers (Kislev)') {
				horseArcherCount += unitArray[i2].count
			}	
			if (unitArray[i2].unit.name === 'Axemen (Kislev)') {
				axemanCount += unitArray[i2].count
			}			
		}	
		...
		for (i2 = 0; i2 < unitsInArmy.length; i2++) {
			...
			if (
				horseArcherCount < wouldBeMaximumCount * 2 ||
				axemanCount < wouldBeMaximumCount * 2
			) {
				locked = true
			}
			if (unitsInArmy[i2].name === 'Horse Archers (Kislev)') {
				if (axemanCount < wouldBeMaximumCount * 2) {
					locked = true
				} else {
					locked = false
				}
			}
			if (unitsInArmy[i2].name === 'Axemen (Kislev)') {
				if (horseArcherCount < wouldBeMaximumCount * 2) {
					locked = true
				} else {
					locked = false
				}
	...
	determineIfValidAfterPointIncrease(pointIncrease) {
		...
		if (maximumCountBefore === maximumCountAfter) {
			...
			for (i2 = 0; i2 < listedUnits.length; i2++) {
				if (listedUnits[i2].unit.name === 'Horse Archers (Kislev)') {
					horseArcherCount += listedUnits[i2].count
				}	
				if (listedUnits[i2].unit.name === 'Axemen (Kislev)') {
					axemanCount += listedUnits[i2].count
				}			
			}
			if (
				horseArcherCount > maximumCountAfter * 2 ||
				axemanCount > maximumCountAfter * 2
			) {
				valid = false
			} else {
				valid = true
	...
	removeUnit(unitToRemove) {
		...
		for (i2 = 0; i2 < listedUnits.length; i2++) {
			if (listedUnits[i2].unit.name === unitToRemove.unit.name) {
				if (
					unitToRemove.unit.name === 'Horse Archers (Kislev)' ||
					unitToRemove.unit.name === 'Axemen (Kislev)'
				) {
...
export default KislevContainer