...
class ArabyContainer extends Component {
	constructor(props) {
		super(props)
		this.state = {
			listedUnits: [
	...
	determineIfGreyedOut(unitArray) {
		...
		for (i2 = 0; i2 < unitArray.length; i2++) {
			...
			if (unitArray[i2].unit.name === 'Bowmen (Araby)') {
				bowmanCount += unitArray[i2].count
			}			
		}	
		...
		for (i2 = 0; i2 < unitsInArmy.length; i2++) {
			...
			if (
				...
				bowmanCount < wouldBeMaximumCount * 2
			) {
				locked = true
			}
			if (unitsInArmy[i2].name === 'Spearmen (Araby)') {
				if (bowmanCount < wouldBeMaximumCount * 2) {
					locked = true
				} else {
					locked = false
				}
			}
			if (unitsInArmy[i2].name === 'Bowmen (Araby)') {
	...
	determineIfValidAfterPointIncrease(pointIncrease) {
		...
		if (maximumCountBefore === maximumCountAfter) {
			...
			for (i2 = 0; i2 < listedUnits.length; i2++) {
				if (listedUnits[i2].unit.name === 'Spearmen (Araby)') {
					spearmanCount += listedUnits[i2].count
				}	
				if (listedUnits[i2].unit.name === 'Bowmen (Araby)') {
					bowmanCount += listedUnits[i2].count
				}			
			}
			if (
				spearmanCount > maximumCountAfter * 2 ||
				bowmanCount > maximumCountAfter * 2
			) {
				valid = false
			} else {
				valid = true
	...
	removeUnit(unitToRemove) {
		...
		for (i2 = 0; i2 < listedUnits.length; i2++) {
			if (listedUnits[i2].unit.name === unitToRemove.unit.name) {
				if (
					unitToRemove.unit.name === 'Spearmen (Araby)' ||
					unitToRemove.unit.name === 'Bowmen (Araby)'
				) {
