...
class DogsOfWarContainer extends Component {
	constructor(props) {
		super(props)
		this.state = {
			listedUnits: [
	...
	determineIfGreyedOut(unitArray) {
		...
		for (i2 = 0; i2 < unitsInArmy.length; i2++) {
			...
			if (
				pikemanCount < wouldBeMaximumCount * 2 ||
				crossbowmanCount < wouldBeMaximumCount * 2
			) {
				locked = true
			}
			if (unitsInArmy[i2].name === 'Pikemen (Dogs of War)') {
				if (crossbowmanCount < wouldBeMaximumCount * 2) {
					locked = true
				} else {
					locked = false
				}
			}
			if (unitsInArmy[i2].name === 'Crossbowmen (Dogs of War)') {
				if (pikemanCount < wouldBeMaximumCount * 2) {
					locked = true
				} else {
					locked = false
				}
	...
	determineIfValidAfterPointIncrease(pointIncrease) {
		...
		if (maximumCountBefore === maximumCountAfter) {
			...
			for (i2 = 0; i2 < listedUnits.length; i2++) {
				if (listedUnits[i2].unit.name === 'Pikemen (Dogs of War)') {
					pikemanCount += listedUnits[i2].count
				}	
				if (listedUnits[i2].unit.name === 'Crossbowmen (Dogs of War)') {
					crossbowmanCount += listedUnits[i2].count
				}			
			}
			if (
				pikemanCount > maximumCountAfter * 2 ||
				crossbowmanCount > maximumCountAfter * 2
			) {
				valid = false
			} else {
				valid = true
	...
	removeUnit(unitToRemove) {
		...
		for (i2 = 0; i2 < listedUnits.length; i2++) {
			if (listedUnits[i2].unit.name === unitToRemove.unit.name) {
				if (
					unitToRemove.unit.name === 'Pikemen (Dogs of War)' ||
					unitToRemove.unit.name === 'Crossbowmen (Dogs of War)'
				) {
...
export default DogsOfWarContainer