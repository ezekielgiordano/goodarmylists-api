	...
	determineIfGreyedOut(unitArray) {
		...
		for (i2 = 0; i2 < unitsInArmy.length; i2++) {
			...
			if (
				...
				crossbowmanCount < wouldBeMaximumCount
			) {
				locked = true
			}
			if (unitsInArmy[i2].name === 'Spearmen (Dark Elves)') {
				if (crossbowmanCount < wouldBeMaximumCount) {
					locked = true
				} else {
					locked = false
				}
			}
			if (unitsInArmy[i2].name === 'Crossbowmen (Dark Elves)') {
	...
	determineIfValidAfterPointIncrease(pointIncrease) {
		...
		if (maximumCountBefore === maximumCountAfter) {
			...
			for (i2 = 0; i2 < listedUnits.length; i2++) {
				if (listedUnits[i2].unit.name === 'Spearmen (Dark Elves)') {
					spearmanCount += listedUnits[i2].count
				}	
				if (listedUnits[i2].unit.name === 'Crossbowmen (Dark Elves)') {
					crossbowmanCount += listedUnits[i2].count
				}			
			}
			if (
				spearmanCount > maximumCountAfter * 2 ||
				crossbowmanCount > maximumCountAfter
			) {
				valid = false
			} else {
				valid = true
	...
	removeUnit(unitToRemove) {
		...
		for (i2 = 0; i2 < listedUnits.length; i2++) {
			if (listedUnits[i2].unit.name === unitToRemove.unit.name) {
				...
				if (unitToRemove.unit.name === 'Crossbowmen (Dark Elves)') {
					if (listedUnits[i2].count - 1 >= wouldBeMaximumCount) {
						listedUnits[i2].count -= 1
					}
				}				
				if (
					unitToRemove.unit.name !== 'Spearmen (Dark Elves)' &&
					unitToRemove.unit.name !== 'Crossbowmen (Dark Elves)' &&
