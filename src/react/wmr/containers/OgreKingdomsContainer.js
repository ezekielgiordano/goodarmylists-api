...
class OgreKingdomsContainer extends Component {
	constructor(props) {
		super(props)
		this.state = {
			listedUnits: [
	...
	determineIfGreyedOut(unitArray) {
		...
		for (i2 = 0; i2 < unitsInArmy.length; i2++) {
			...
			if (bullOgreCount < wouldBeMaximumCount * 2) {
				locked = true
			}
			if (unitsInArmy[i2].name === 'Bull Ogres (Ogre Kingdoms)') {
				locked = false
			}
			if (locked === true) {
	...
	determineIfValidAfterPointIncrease(pointIncrease) {
		...
		if (maximumCountBefore === maximumCountAfter) {
			...
			for (i2 = 0; i2 < listedUnits.length; i2++) {
				if (listedUnits[i2].unit.name === 'Bull Ogres (Ogre Kingdoms)') {
					bullOgreCount += listedUnits[i2].count
				}			
			}
			if (bullOgreCount > maximumCountAfter * 2) {
				valid = false
			} else {
				valid = true
			}
	...
	removeUnit(unitToRemove) {
		...
		for (i2 = 0; i2 < listedUnits.length; i2++) {
			if (listedUnits[i2].unit.name === unitToRemove.unit.name) {
				...
				if (
					unitToRemove.unit.name !== 'Bull Ogres (Ogre Kingdoms)' &&
...
export default OgreKingdomsContainer