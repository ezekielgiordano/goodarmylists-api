...
class DemonsContainer extends Component {
	constructor(props) {
		super(props)
		this.state = {
			listedUnits: [
	...
	determineIfGreyedOut(unitArray) {
		...
		for (i2 = 0; i2 < unitsInArmy.length; i2++) {
			...
			if (demonHordeCount < wouldBeMaximumCount * 3) {
				locked = true
			}
			if (unitsInArmy[i2].name === 'Demon Hordes (Demons)') {
				locked = false
			}
			if (locked === true) {
	...
	determineIfValidAfterPointIncrease(pointIncrease) {
		...
		if (maximumCountBefore === maximumCountAfter) {
			...
			for (i2 = 0; i2 < listedUnits.length; i2++) {
				if (listedUnits[i2].unit.name === 'Demon Hordes (Demons)') {
					demonHordeCount += listedUnits[i2].count
				}				
			}
			if (demonHordeCount > maximumCountAfter * 3) {
				valid = false
			} else {
				valid = true
			}
	...
	removeUnit(unitToRemove) {
		...
		for (i2 = 0; i2 < listedUnits.length; i2++) {
			if (listedUnits[i2].unit.name === unitToRemove.unit.name) {
				...
				if (
					unitToRemove.unit.name !== 'Demon Hordes (Demons)' &&
...
export default DemonsContainer